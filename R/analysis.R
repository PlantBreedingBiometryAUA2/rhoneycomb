...
analysis<-function(Main_Data_Frame=NULL,Response_Vector=NULL,ring=6,blocks=FALSE,row_element=NULL,plant_element=NULL,CRS=5){
    ...
    if(blocks==TRUE){ 
        ...
        #Add a grand total (make a list with the first two data frames plus the grand total)
        #also round the results at 4 digits
        Main_Data_Frame<-list(round(Main_Data_Frame[[1]],4),round(Main_Data_Frame[[2]],4),"Grand_Mean"=round(mean(Main_Data_Frame[[1]]$Data,na.rm=TRUE),4))
        return(Main_Data_Frame)
    }
    ...
    #Round the result and add a grand mean
    return_value<-list(round(return_value[[1]],4),      round(return_value[[2]],4),  "Grand_Mean"=round(mean(return_value[[1]]$Data,na.rm=TRUE),4)       )
    return(return_value)
}
